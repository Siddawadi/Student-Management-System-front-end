...
import type { updatecourseINput } from '../../types/course.types'
...
export const Updatecourse = ({id}:Istring) => {
  ...
  const { mutate, isPending } = useMutation({  
    mutationFn: (data: updatecourseINput) => updateCourse(id, data), // ✅ calls API function
    onSuccess: () => {
  ...
  const onSubmit: SubmitHandler<updatecourseINput> = (formdata) => {
    mutate(formdata) // ✅ mutate called here
  }
    ...
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='min-h-10 min-w-full py-10'>
          <button
            disabled={isPending}
            type='submit' // ✅ only type='submit', no onClick needed
